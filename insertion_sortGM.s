...
insertion_sort:
	...
	dec %rcx					# bo petla rozpoczyna sie od 1

	mov $1, %rdx					# to jest nasze i - zewnetrzny licznik

	mov $numbers, %rdi				# zapisujemy kolejna wartosc z tabeli jako klucz

	_loop_is:

		movl numbers(,%rdx,4),%eax			# key = arr[i]
		...
		_loop_echg_is:
			...
			cmp (%rdi,%rsi),%eax	# arr[j] > key ? : czy klucz  nadal mniejszy od elementu tabeli?
			jae _exit_loop_exchg_is	# jesli 2 warunek nie jest zachowany wychodzimy
		...
		movl %eax, numbers(,%rsi,4)		# arr[j+1] = key
