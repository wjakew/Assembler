...
insertion_sort:
	...
	mov %r8, %rcx							# zapisujemy licznik obrotow petli

	_loop_is:
			mov $numbers, %rbx				# zapisujemy kolejna wartosc z tabeli jako klucz

			mov %rcx,%rsi					# przepisuje licznik obrotow do %rsi w celu przesuniecia

			inc %rsi						# to jest nasze j

			# w tym stanie %rsi -> j !!!! %rax -> klucz

			push %rcx

			mov %rsi, %rcx					# przenosze aktualne j do licznika obrotow petli
			...
			_loop_2_is:
						...
						cmp (%rsi,%rbx),%rbx					# 2 warunek istnienia petli
						
						ja _exit_loop_2_is						# jesli 2 warunek nie jest zachowany wychodzimy


						mov (%rsi,%rbx), %rax					# kopiuje liczbe do raxa bo za duzo referencji 

						mov (%rax), %rdi							# przenosze aktualna liczbe do %rdi

						xchg 4(%rsi,%rbx),%rax					# zastepuje aktualna liczbe nastepna

						mov %rdi, 4(%rsi,%rbx)					# przywracam z rdi (gdzie jest poprzednia liczba) na nastepne miejsce w tabeli

						_exit_loop_2_is:			# jesli nie ma drugiego warunku istnienia petli od razu wychodzimy
							jmp _loop_is
			...
			_not_loop_2_is:
				...
				pop %rcx							# przywracam poprzedni licznik petli

				mov 4(%rbx),%rbx					# przesuwam na kolejna wartosc z tabeli numbers
