...
selection_sort:
	...
	mov $0, %rdx									# %rdx -> i = 0
	mov $numbers,%r8								# %r8 -> $numbers

	mov -1(%r9),%rdi

	dec %rdi										# n-1 (potrzebne do petli)

	_loop_1_ss:

		mov %rdx,%rsi								# min_idx = i

		mov %rdx,%rcx								# j = i

		inc %rdx									# j++ bo j=i+1 (wartosc poczatkowa petli)
		...
		_loop_2_ss:
			...
			mov (%rdx,%r8),%rax
			cmp %rax,(%rsi,%r8)				#if (arr[j] < arr[min_idx])
			jae _loop_2_ss
			mov %rdx,%rsi							# min_idx = j

			inc %rdx								# j++
			jmp _loop_2_ss							# kontynuacja obrotow petli 2 (wewnetrznej)
		...
		xor %rax,%rax
		mov numbers(,%rsi,4),%rax
		xchg %rax, numbers(,%rdx,4)		#swap(&arr[min_idx],&arr[i])
		inc %rdx									#i++
		jmp _loop_1_ss								# kontynuacja obrotow petli 1 (zewnetrznej)
