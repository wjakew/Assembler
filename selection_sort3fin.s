...
# %rdi -> iMin
# %rsi -> j
# %r8 -> rejestr pomocniczy do przenoszenia
# %r9 -> n		(ilosc liczb w tablicy)
# $numbers -> tablica liczb
selection_sort:
	...
	_loop_ss_1:
		...
		#sprawdzanie warunku obrotu petli 1
		cmp %rcx,%rsi		# n-1>j
		jae _loop_ss_1_exit
		...
		#sprawdzanie warunku obrotu petli 2
		cmp %r9,%rbx			#n>i
		jae _loop_ss_2_exit

		mov numbers(,%rbx,4),%r8	#pomocnicze przeniesienie bo za duzo odwolan do pamieci
		cmp numbers(,%rdi,4),%r8	#a[i]<a[iMin]
		jae _continue_w
		...
		#zamiana liczb na pozycjach
		#swap(a[j],a[iMin])

		mov numbers(,%rdi,4), %r8	#zapisujemy a[iMin] w %r8
		mov numbers(,%rsi,4),%r10	#zapisujemy a[j] w %r10
		mov %r8,numbers(,%rsi,4)	#zapisujemy to co jest w a[iMin] w a[j]
		mov %r10,numbers(,%rdi,4)	#zapisujemy stara wartosc a[j] na a[iMin] 
